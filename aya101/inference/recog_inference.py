...
@torch.no_grad()
def generate_combined_gating(model_hi, model_en, tokenizer, gating_model,
    ...
    for step in range(max_length):
        ...
        hid_hi = torch.cat([layer[:, -1, :] for layer in out_hi.decoder_hidden_states[-3:]], dim=-1)
        hid_en = torch.cat([layer[:, -1, :] for layer in out_en.decoder_hidden_states[-3:]], dim=-1)
...
def main():
    ...
    output = generate_combined_gating(
                ...
                sentence_hi="आपका नाम क्या है?", sentence_en="What is your name?", prompt_hi=prompt_hi, prompt_en=prompt_en)
